...
export default function CardIncommon(props) {

  const [isPending,setIsPending] = useState(false)
  const [wear,setWear] = useState(40)

  //setFuel
  const [fuel,setFuel] = useState([true,true,true,true,'extra'])
  ...
  return (
      <div className="cards p-2 max-w-[348px] w-full">
          {isPending && <Loading/>}
          <div className="flex justify-center uncommon rounded-[20px]">
          <img src={props.image} className="max-w-[242px] w-full h-auto py-[24px] px-[46px]" alt="card incommon"/>
          </div>
